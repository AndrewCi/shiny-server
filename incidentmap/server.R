...
server <- function(input, output, session) {
  ...
  observe({
    ...
    leafletProxy("map", data = data_current) %>% clearControls() %>% clearMarkers() %>% removeControl("legend1") %>% addCircleMarkers(radius = 1, weight = 4, opacity = 1, fill = FALSE, color = ~pal(data_current$Score), popup = paste("<br><b>Date:</b> ", data_current$Incident.Date, "</br><br><b>Type:</b> ", data_current$Type, "</br><br><b>Road Name:</b> ", data_current$Road.Name,"</br>",data_current$Score)) 
    }else{
